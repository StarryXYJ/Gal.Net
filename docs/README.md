...
| **IGameView** | Runtime 与 UI 的唯一契约接口（组合接口） |
| **GameRuntime** | 运行时状态容器（位置/变量/场景/调用栈/Settings） |
| **IGameGraphCompiler** | 编译管道抽象（ComplexEntry → SimpleEntry） |
| **EntryHandler** | 条目执行逻辑的扩展点 |

### 运行时流程

```
Graph → IGameGraphCompiler 编译 → 状态机循环 → 组执行完 → 沿边转移
                                      ↓
                               Handler.Start(ctx)
...
| [运行时参考](spec/runtime.md) | GameRuntime、GameEngine、GameSnapshot、VariableStore、ExpressionEvaluator、IGameGraphCompiler、EntryHandlerRegistry |
