...
internal sealed class ReorderDragController
{
    ...
    public ReorderDragController(ListBox listBox, AvaloniaControl owner, Action<int, int> onMove)
    {
        _listBox = listBox;
        _owner = owner;
        _onMove = onMove;
    ...
    public void AttachOverlay(Panel panel)
    {
        if (_overlay.Parent == panel)
            return;

        if (_overlay.Parent is Panel existing)
            existing.Children.Remove(_overlay);
